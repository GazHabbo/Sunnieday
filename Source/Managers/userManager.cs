...
namespace Holo.Managers
{
    ...
    public static class userManager
    {
        ...
        private static Dictionary<string, int> _UserNameIds = new Dictionary<string, int>();
            ...
            lock (_Users)
            {
                ...
                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                {
                    ...
                    if (User.connectionRemoteIP == dbClient.getString("SELECT ipaddress_last FROM users WHERE name = @name LIMIT 1"))
                    {
                        ...
                        if (!_UserNameIds.ContainsKey(User._Username))
                        {
                            _UserNameIds.Add(User._Username, userID);
                        }
                    ...
                    else
                    {
                        ...
                        User.sendNotify("Invalid Session Ticket, please login again!");
                    }
        ...
        public static int getUserID(string userName)
        {
            lock (_UserNameIds)
            {
                if (_UserNameIds.ContainsKey(userName))
                    return _UserNameIds[userName];
                else
                {
                    int id;
                    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                    {
                        ...
                        id = dbClient.getInt("SELECT id FROM users WHERE name = @name LIMIT 1");
                    }
                    _UserNameIds.Add(userName, id);
                    return id;
                }
            }
        ...
        {
            if (_Users.ContainsKey(userID))
                return ((virtualUser)_Users[userID])._Username;
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
        ...
        /// </summary>
        /// <param name="userID">The ID of the user to get the friend ID's from.</param>
        public static Dictionary<int, virtualBuddy> getUserFriendIDs(int userID)
        {
            Dictionary<int, virtualBuddy> userList = new Dictionary<int, virtualBuddy>();
            try
            {
                DataTable dTable;
                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                {
                    dTable = dbClient.getTable("SELECT u.name AS user_name, u.id AS id, u.mission AS mission, u.lastvisit AS last_online " +
                                                "FROM messenger_friendships " +
                                                "INNER JOIN users AS u ON u.id = userid " +
                                                "WHERE " + userID + " = friendid " +
                                                "UNION ALL " +
                                                "SELECT u.name AS user_name, u.id AS id, u.mission AS mission, u.lastvisit AS last_online " +
                                                "FROM messenger_friendships " +
                                                "INNER JOIN users AS u ON u.id = friendid " +
                                                "WHERE " + userID + " = userid");
                }
                string mission;
                string lastOnline;
                ...
                {
                    userid = Convert.ToInt32(dRow["id"]);
                    mission = dRow["mission"].ToString();
                    name = dRow["user_name"].ToString();
                    lastOnline = dRow["last_online"].ToString();
                    userList.Add(userid, (new virtualBuddy(userid, mission, lastOnline, name)));
                }
        ...
        public static void sendData(string Data)
        {
            try
            {
                lock (_Users)
                {
                    Hashtable alerters = (Hashtable)_Users.Clone();
                    foreach (virtualUser User in alerters.Values)
                    {
                        try { User.sendData(Data); }
                        catch { Out.WritePlain("Gebruiker " + User._Username + " is offline"); }
                    }
                }
            }
            catch { }
        ...
        public static string generateCommands(byte Rank, List<string> fuseRights)
        {
            ...
            if (Rank > 1)
            {
                ...
                if (rankManager.containsRight(Rank, "fuse_alert", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":alert NAME MESSAGE - Sends a moderator alert%";
                }

                if (rankManager.containsRight(Rank, "fuse_room_alert", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":roomalert MESSAGE - Sends a moderator alert to a whole room%";
                }

                if (rankManager.containsRight(Rank, "fuse_kick", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":kick [MESSAGE] - Kicks a user from the room they are in%";
                }

                if (rankManager.containsRight(Rank, "fuse_room_kick", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":roomkick [MESSAGE] - Kicks all users but you from the room";
                }

                if (rankManager.containsRight(Rank, "fuse_mute", fuseRights))
                {
                    Cmds += "%";
                ...
                if (rankManager.containsRight(Rank, "fuse_room_mute", fuseRights))
                {
                    Cmds += "%";
                ...
                if (rankManager.containsRight(Rank, "fuse_ban", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":ban NAME HOURS REASON - Bans a user for the how many hours%";
                }

                if (rankManager.containsRight(Rank, "fuse_superban", fuseRights))
                {
                    Cmds += "%";
                    Cmds += ":superban NAME HOURS REASON - Bans a user and there IP for how many hours%";
                }

                if (rankManager.containsRight(Rank, "fuse_administrator_access", fuseRights))
                {
                    Cmds += "%";
                ...
                if (rankManager.containsRight(Rank, "fuse_teleport", fuseRights))
                {
                    Cmds += "%";
                ...
                if (rankManager.containsRight(Rank, "fuse_moderator_access", fuseRights))
                {
                    Cmds += "%";
        ...
        public static string generateWhosOnline(bool Admin)
        {
            ...
            int p = 1;
            if (Admin)
                foreach (KeyValuePair<int, virtualUser> user in _Users)
                {
                    if (r == 0)
                    {
                        p++;
                    ...
                    packet.Append("\r" + getUserName(user.Key) + " [" + user.Key.ToString() + "] {" + getUser(user.Key)._Rank + "}   " + getUser(user.Key).connectionRemoteIP);
                    r--;
                }
            else
            {
                foreach (KeyValuePair<int, virtualUser> user in _Users)
                {
                    ...
                    packet.Append("\r" + getUserName(user.Key) + " {" + getUser(user.Key)._Rank + "}");
                    r--;
                }
            }
            return packet.ToString();
        ...
        public static void setBan(string IP, int Hours, string Reason, int ID)
        {
            string Expires = DateTime.Now.AddHours(Hours).ToString().Replace("/", "-").Replace(".", "-");
            DataColumn dCol;
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                ...
                dbClient.runQuery("INSERT INTO users_bans (ipaddress,date_expire,descr,userid) VALUES (@ip,@expires,@reason,@ID)");
        ...
        public static string generateBanReport(int userID)
        {
            try
            {
                ...
                else
                {
                    ...
                    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                    {
                        DRow = dbClient.getRow("SELECT userid,note,timestamp FROM system_stafflog WHERE action = 'ban' AND targetid = '" + userID + "' "); // Get latest stafflog entry for this action (if exists)
                    }
        ...
        private static void checkPings()
        {
            try
            {
                List<virtualUser> deadConnectionList = new List<virtualUser>();
                while (true)
                {
                    try
                    {
                    ...
                    foreach (virtualUser User in deadConnectionList)
                    {
                        Holo.Out.WriteLine(User._Username + " timed out.");
                        User.Disconnect();
                    }
                    deadConnectionList.Clear();
        ...
        public static void disconnectAllUsers()
        {
            ...
            lock (_Users)
            {
                ...
                virtualUser[] users = new virtualUser[_Users.Count];
                int x = 0;
                foreach (virtualUser v in _Users.Values)
                {
                    users[x] = v;
                    x++;
                }
        ...
        /// <summary>
        /// Creates a genie with a specifix state
        /// </summary>
        internal static void makeMassGeest()
        {
            Hashtable holos = (Hashtable)_Users.Clone();
            foreach (virtualUser user in holos.Values)
                user.makeGenie(state);

        }
        internal static void serverSave()
        {
            lock (_Users)
            {
