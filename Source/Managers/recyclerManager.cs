...
namespace Holo.Managers
{
    ...
    public static class recyclerManager
    {
        ...
        public static void Init(bool Update)
        {
            try
            {
                if (Config.getTableEntry("recycler_enable") == "1")
                {
                    ...
                    {
                        itemTemplate Template = catalogueManager.getTemplate(Convert.ToInt32(dRow["rclr_reward"]));
                        if (Template.Sprite != "")
                        {
        ...
        public static List<genericItem> rewardSession(int userID)
        {
            ...
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                dbClient.AddParamWithValue("reward", rewardTemplateID);
                dbClient.AddParamWithValue("userID", userID);
                int lastItem = (int)dbClient.insertQuery("INSERT INTO furniture(tid) VALUES (@reward)");
                itemlist.Add(new genericItem(lastItem, rewardTemplateID, "", ""));
            }
            return itemlist;
        }
        /// <summary>
