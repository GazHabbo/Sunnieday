...
namespace Holo.Managers
{
    ...
    public static class soundMachineManager
    {
        ...
        public static string getMachineSongList(int machineID)
        {
            ...
            {
                dTable = dbClient.getTable("SELECT id,length,title,burnt FROM soundmachine_songs WHERE machineid = '" + machineID + "' ");
            }
        ...
        public static string getMachinePlaylist(int machineID)
        {
            ...
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                dCol = dbClient.getColumn("SELECT songid FROM soundmachine_playlists WHERE machineid = '" + machineID + "' ");
            }
