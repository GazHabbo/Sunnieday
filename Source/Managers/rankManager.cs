...
namespace Holo.Managers
{
    ...
    public static class rankManager
    {
        ...
        public static void Init(bool Update)
        {
            ...
            {
                dTable = dbClient.getTable("SELECT title, minpoints, maxpoints FROM games_ranks WHERE type = 'bb' ");
            }
            ...
            using (DatabaseClient dbClient = Eucalypt.criticalManager.GetClient())
            {
                dTable = dbClient.getTable("SELECT title, minpoints, maxpoints FROM games_ranks WHERE type = 'ss' ");
            }
            ...
            using (DatabaseClient dbClient = Eucalypt.criticalManager.GetClient())
            {
                dTable = dbClient.getTable("SELECT title, minpoints, maxpoints FROM games_ranks WHERE type = 'ws' ");
            }
            ...
            Out.WriteLine("Loaded " + gameRanksWS.Length + " ranks for game 'Wobble Squabble'.");
        ...
        public static string fuseRights(byte rankID, Holo.Virtual.Users.virtualUser user)
        {
            ...
            foreach (string fuseright in user._fuserights)
        ...
        /// <param name="userID">The ID of the user.</param>
        public static bool containsRight(byte rankID, string fuseRight, System.Collections.Generic.List<string> fuseRights)
        {
            if (fuseRights.Contains(fuseRight))
                return true;
