...
namespace Holo.Socketservers
{
    ...
    public static class musSocketServer
    {
        ...
        private static bool firstPacket= true;
        /// <summary> 
        ...
        internal static bool Init(int bindPort, string musHost)
        {
            ...
            catch (Exception e) { Out.writeSeriousError(e.ToString()); Eucalypt.Shutdown(); return false; }
        }
            ...
            private void dataArrival(IAsyncResult iAr)
            {
                try
                {
                    ...
                    switch (musHeader)
                    {
                        ...
                        case "UPHA":
                            {
                                lock (userManager._Users)
                                {
                                    if (Data.Length > 4)
                                    {
                                        userManager.sendData("BK"+ stringManager.getString("scommand_hotelalert") + "\r" + Data.Substring(4));
                        ...
                        case "UPRC": // User profile - reload credits
                            {
                                int userID = int.Parse(musData[0]);
                                userManager.getUser(userID).refreshValueables(true, false, false, true);
                                break;
                            }

                        case "UPRX": // User profile - reload every-one's credits
                            {

                                lock (userManager._Users)
                                {
                                    foreach (virtualUser user in userManager._Users.Values)
                                        user.refreshValueables(true, false, false, true);
                                }
                                break;
                            }
                        ...
                        case "UPRT": // User profile - reload tickets
                            {
                                int userID = int.Parse(musData[0]);
                                if (userManager.containsUser(userID))
                                    userManager.getUser(userID).refreshValueables(false, true, false, true);
                                break;
                            }
                        ...
                        case "UPRH":// User profile - reload hand
                            {
                                ...
                                if (userManager.containsUser(userID))
                                {
                                    userManager.getUser(userID).generateHandItems();
