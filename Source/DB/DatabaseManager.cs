...
namespace Ion.Storage
{
    ...
    public class DatabaseManager
    {
        ...
        public DatabaseManager(DatabaseServer pServer, Database pDatabase, bool includeCycle)
        {
            mServer = pServer;
            mDatabase = pDatabase;
            this.includeCycle = includeCycle;
        }
        /// <summary>
        ...
        public DatabaseManager(string sServer, uint Port, string sUser, string sPassword, string sDatabase, uint minPoolSize, uint maxPoolSize, bool includeCycle)
        {
            try
            {
                Out.WriteRedText(maxPoolSize + "");
                ...
                while (includeCycle)
                {
                    lock (this)
                    {
                        ...
                        for (int i = 0; i < mClients.Length; i++)
                        {
                            ...
                            {
                                if (mClients[i].Inactivity >= 300) // Not used in the last 30 seconds
                                {
